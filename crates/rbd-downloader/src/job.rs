...
impl JobState {
    ...
    pub fn transition(&self, next: &JobState) -> Result<JobState, String> {
        match (self, next) {
            // Pending 可进入 Running 或直接取消
            (JobState::Pending, JobState::Running) => Ok(JobState::Running),
            (JobState::Pending, JobState::Cancelled) => Ok(JobState::Cancelled),

            // Running 可暂停/取消/完成/失败
            (JobState::Running, JobState::Paused) => Ok(JobState::Paused),
            (JobState::Running, JobState::Cancelled) => Ok(JobState::Cancelled),
            (JobState::Running, JobState::Done) => Ok(JobState::Done),
            (JobState::Running, JobState::Failed(_)) => Ok(next.clone()),

            // Paused 可恢复或取消
            (JobState::Paused, JobState::Running) => Ok(JobState::Running),
            (JobState::Paused, JobState::Cancelled) => Ok(JobState::Cancelled),
